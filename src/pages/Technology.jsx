...
export default function Technology() {
  ...
  const copyToClipboard = (text, index) => {
    // Extraer solo el código de los bloques de código
  ...
  const renderContent = (contentSection) => {
    ...
    // Unir todo el contenido del array en un solo string
    const fullText = contentSection.join('\n');
  
    // Dividir en texto y bloques de código
    ...
    return parts.map((part, index) => {
      ...
      // Índices impares: bloque de código
      return (
  ...
  return (
    ...
    >
      {/* Sidebar */}
      <aside className="w-64 bg-slate-900 p-6 border-r border-slate-700">
        <h2 className="text-xl font-bold mb-6 capitalize text-transparent bg-clip-text bg-gradient-to-r from-white to-blue-200">{techId.replace(/-/g, " ")}</h2>
      ...
      <main className="flex-1 p-8">
        <span className="flex gap-6 items-center mb-6">
          {/* Botón Atrás */}
          <button
